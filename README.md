...
#### 3-3. 코드 리뷰 하네스 — `/harness` ⭐ NEW (v1.6.0)

Java 코드 또는 SQL을 입력하면 **Analyst → Builder → Reviewer** 3단계 AI 파이프라인이 자동으로 실행됩니다.

| 기능 | 설명 |
|------|------|
| **3단계 파이프라인** | 분석가(Analyst) → 개선가(Builder) → 검토자(Reviewer) 순차 실행 |
| **언어 선택** | Java / SQL 탭 전환 |
...
| **이력 저장** | 분석 결과가 리뷰 이력에 자동 저장 (`HARNESS_REVIEW` 타입) |
...
Spring MVC Controllers (claude-toolkit-ui)
    ...
    │  HarnessController         /harness          3단계 파이프라인 + Diff 뷰              ★ NEW
    │  ReviewHistoryController   /history          이력 관리 + 재실행 버튼 (v0.9)       ★ UPDATED
...
Service Layer
    ...
    │  HarnessReviewService       — Analyst·Builder·Reviewer 파이프라인          ★ NEW
...
└── claude-toolkit-ui/
    ...
    └── src/main/resources/
        ...
        └── templates/
            ...
            ├── history/index.html                  # 재실행 버튼 추가                            ★ UPDATED (v0.9)
