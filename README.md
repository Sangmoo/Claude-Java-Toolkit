...
- 🛡 **Settings 변경 감사 로그 (#15, ADMIN 전용 신규 페이지)** — `/admin/config-changes`
  - 22개 시스템 설정 (DB / Claude / scanPath / SMTP / rate-limit 등) 변경 시
    이전값/새값/변경자/IP/시각을 `config_change_log` 테이블에 자동 기록
  - 외부감사인이 "settings 가 언제 누구에 의해 어떻게 바뀌었는지" 1쿼리로 추적
  - 패스워드/API 키 등 민감 필드는 자동 마스킹 (`***` 로 저장 — 평문 보관 X)

- ⚡ **파일 해시 기반 분석 캐시 (#18)** — `AnalysisCacheService` 4-arg key + 정규화
  - 입력 정규화: BOM 제거 / CRLF→LF / trailing whitespace / 연속 빈 줄 축약
    → 같은 의미의 입력은 항상 같은 캐시 키로 매핑 (체감 적중률 향상)
  - 캐시 키: `feature + input + input2 + sourceType` 4개 SHA-256 해싱
    → `sql_review` + 같은 SQL 이라도 `review` vs `security` 옵션이 다르면 분리 (충돌 버그 회귀 방지)
  - JVM 라이프타임 hit/miss 카운터 + 적중률 (`/api/v1/admin/cache/stats`)
  - 모든 분석 페이지 결과 헤더에 **⚡ 캐시 적중** 칩 (캐시로 즉시 반환된 경우, Claude API 비용 0)

- 🏷 **리뷰 이력 태깅 시스템 (#12)** — `/history` 페이지 확장
  - 각 분석 결과에 콤마 구분 태그 (예: `성능, SLA위반, DB`) — 본인 또는 ADMIN 만 수정
  - 헤더 dropdown 에서 빈도순 태그 빠른 선택 / 클릭으로 토글 필터
  - 카드 행에 태그 칩 + 확장 영역에 자동완성 dropdown (Enter 저장 / Esc 취소)
  - 태그 정규화: trim / 빈·중복 제거 / 30자 상한, 콤마 매칭 충돌 방지(`,DB,` 패턴)

- 💬 **결과 후속 질문 패널 — Follow-up Q&A (#4)** — 22+ 분석 페이지에 한 번에 적용
  - 분석 결과 직후 같은 화면에서 "왜 이 결과가 나왔나요?" / "PostgreSQL 에서는?" 등
    추가 질문 가능 — 페이지 이동 없이 결과를 보면서 질문
  - 기존 `/chat/send` + `/chat/stream` SSE 인프라 그대로 재사용 (백엔드 변경 0)
  - 결과 + 원본 입력 + 페이지 라벨을 첫 질문 시 1회 컨텍스트로 전달, 이후 멀티턴
  - 결과가 변경되면 (재분석) 메시지/세션 자동 리셋

- 🔗 **분석 체이닝 — 결과 후 다음 단계 1-클릭 (#2)** — 22+ 분석 페이지에 한 번에 적용
  - 14개 feature × 평균 3개 다음단계 매핑 (총 ~40개 체이닝 경로)
    `sql_review → /explain → /sql/index-advisor → /sql-translate → /erd` 등
  - sessionStorage 기반 1회용 handoff (60초 TTL), URL 길이 제한·로그 노출 우려 0
...
- 🔌 **Live DB 직접 연결 — EXPLAIN/통계 자동 수집 (#G3, Phase 0~3)** — Oracle + PostgreSQL
  ...
  - DBMS 별 매핑:
    - **Oracle 11g~19c**: `DBA_TABLES`/`ALL_TABLES`/`USER_TABLES` 권한별 fallback,
      `DBA_INDEXES` + `DBA_IND_COLUMNS`, `EXPLAIN PLAN SET STATEMENT_ID + DBMS_XPLAN.DISPLAY`
